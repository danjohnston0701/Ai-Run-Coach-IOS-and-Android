...
- **Post-Run AI Analysis**: Comprehensive run analysis including highlights, struggles, personal bests, improvement tips, and overall performance scores. Enhanced with Garmin data integration for training load assessment, wellness impact analysis, technical running dynamics (cadence, stride length, ground contact time, vertical oscillation), training effect insights, VO2 max trends, and personalized recovery recommendations.
...
- **OpenAI TTS Integration**: High-quality AI voice for pre-run audio briefings using OpenAI's text-to-speech API. Endpoints: `/api/tts/generate` (general TTS), `/api/coaching/pre-run-briefing-audio` (comprehensive pre-run briefing with audio). Uses 'nova' voice for female coach, 'onyx' for male coach. Falls back to device TTS if OpenAI fails. Audio playback uses expo-audio with createAudioPlayer for imperative audio control.
- **Enhanced Elevation Insights**: Pre-run audio briefings include detailed terrain analysis based on elevation categories: >150m (very challenging with pacing advice), >100m (challenging hills with stride tips), >50m (rolling terrain), >20m (minor undulations), <20m (flat). Includes both elevation gain and loss descriptions.
...
- **Automatic Garmin Sync Scheduler**: Background scheduler (server/scheduler.ts) automatically syncs wellness data from all connected Garmin accounts every 60 minutes. Uses node-cron for scheduling. Runs initial sync 30 seconds after server startup. Syncs sleep, stress, Body Battery, HRV, heart rate, and readiness data.
- **HomeScreen Garmin Indicator**: Displays Garmin connection status prominently above the goals card. Shows "Garmin Connected" with last sync time (e.g., "Just now", "5m ago", "2h ago") when connected, or "Connect Garmin" with tap-to-connect prompt when not connected. Tapping navigates to Connected Devices screen.
